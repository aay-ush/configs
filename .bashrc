...
# TODO: Add color codes from that online source
...
case ${TERM} in
    ...
    screen*)
        ...
        # Allow screen to use $PWD as window title
        PROMPT_COMMAND='echo -ne \033]0;${USER}@${HOSTNAME%%.*}:${PWD/#$HOME/\~}\033\\"'
        set_screen_window_title() {
            printf '\ek%s\e\\' "${PWD/#$HOME/\~}"
        }
        PROMPT_COMMAND="set_screen_window_title; $PROMPT_COMMAND"
        ;;
esac
...
# less for one screen full
alias less='less -F'

# cd faster
alias cd..='cd ..'
alias devdir='cd /media/dhome/dev'
...
# Allow screen't dynamic window title: screen shows current process as window title
# Showing $PWD by modifying PROMPT_COMMAND might be more useful
